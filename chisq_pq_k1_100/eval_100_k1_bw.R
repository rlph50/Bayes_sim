...
for (i in 1:1000) {
  ...
  saveRDS(res,"res_chisq_100_k1_ARMA_bw.RDS")
  cat(paste("Finished ",i,".\n"))
}

res <- readRDS("res_chisq_100_k1_ARMA_bw.RDS")
res$bw_adapt_delta <- 0.8

for (i in 1:1000) if (res$bw_divergent[i]>0|res$bw_lp_rhat[i]>1.1) {
  cat(sprintf("processing y%d.\n",i))
  y<-synProcess100[,paste0("y",i)]
  tt<-system.time(fit<-bayes_whittle(y,control=list(adapt_delta=0.9)))
  res1 <- monitor(extract(fit, permuted = FALSE, inc_warmup = TRUE), print=FALSE) # matrix
  res$bw_t[i]<-tt[['elapsed']]
  res$bw_d1[i] <- res1["d1","mean"]
  res$bw_f1[i] <- res1["f1","mean"]
