...
parameters {
  ...
  real<lower=-0.999,upper=0.999> theta;  // MA coeff
...
model {
  ...
  target += normal_lpdf(theta | 0.0, 0.50);
  ...
  for (i in 1:N) if ((fabs(freq[i]-f1)>0.001)) {
    ...
    spec_den_inv = two_pi * (1.0 + (phi^2) - 2.0*phi*cos_2_pi_f[i] ) / (1.0 + (theta^2) - 2.0*theta*cos_2_pi_f[i] );
