...
GgbrAR1SpecDen<-function(x,ll) {return (cos(ll$k*x)*(4*(cos(x)-cos(2*pi*ll$f1))^2)^(-ll$d1)/(1-2*ll$phi*cos(x)+ll$phi^2));}
...
ll1=list(d1=0.30,f1=1/28,phi=(-0.8),k=0,sigma2=1)

n<-500
system.time(g <- ggbr.sim(n,ll2=ll1))

# # data frame to store the simulated processes
syn_gauss500<-data.frame(obs=1:n)
...
while (i<=1000) {
  ...
  y2<-DHSimulate(2*n,g[1:n])
  syn_gauss500[,paste0("y",i)]<-y2[(n+1):(2*n)]
...
saveRDS(syn_gauss500,'syn_k1gauss500.rds')
