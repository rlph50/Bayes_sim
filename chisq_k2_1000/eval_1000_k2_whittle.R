...
whittle.est<-function(y) {
  ...
  fit<-sim_optim(c(0.25,spikes[['f1']],0.25,spikes[['f2']],0.0),
                 whittle.obj,
                 lower=c(0.01,0.01,0.01,0.01,-0.999),
                 upper=c(0.49,0.40,0.49,0.40, 0.999),
                 ss=ss)
  return(fit$par)
}

synProcess1000 <- readRDS('syn_k2chisq1000.rds')

res <- data.frame(whittle_d1=numeric(1000),whittle_f1=numeric(1000),
                  whittle_d2=numeric(1000),whittle_f2=numeric(1000),
...
for (i in 1:1000) {
  ...
  if (whittle[2]>whittle[4]) {
    res$whittle_d1[i]<-whittle[3]
    res$whittle_f1[i]<-whittle[4]
    res$whittle_d2[i]<-whittle[1]
    res$whittle_f2[i]<-whittle[2]
  } else {
    res$whittle_d1[i]<-whittle[1]
    res$whittle_f1[i]<-whittle[2]
    res$whittle_d2[i]<-whittle[3]
    res$whittle_f2[i]<-whittle[4]
  }

  res$whittle_phi[i]<-whittle[5]
  res$whittle_t[i]<-tt[['elapsed']]
  if ((i%%25)==0) cat(paste("running ",i,".\n"))
}

saveRDS(res,"res_chisq_1000_k2_whittle.RDS")
