...
wll.est<-function(y) {
  ...
  lb<-c(0.01,0.01,-0.999,-0.999)
  ub<-c(0.49,0.40, 0.999, 0.999)
  pars<-c(0.25,spikes[['f1']],0.0,0.0) #initial.est(y)
  fit <- sim_optim(pars, wll.ggbr.obj, lower=lb, upper=ub, ss=ss)
  return(fit$par)  
}

synProcess100 <- readRDS('syn_k1gauss100.rds')

res <- data.frame(wll_d1=numeric(1000),wll_f1=numeric(1000),
                  wll_phi=numeric(1000), wll_theta=numeric(1000),wll_t=numeric(1000))
...
for (i in 1:1000) {
  ...
  tt<-system.time(wll<-wll.est(y))
    ...
    res$wll_f1[i]<-wll[2]
  res$wll_phi[i]<-wll[3]
  res$wll_theta[i]<-wll[4]
  res$wll_t[i]<-tt[['elapsed']]
  saveRDS(res,"res_gauss_100_k1_ARMA_wll.RDS")
