...
## Using an alternative prior - a uniform prior for the AR Phi paramaeter

library(rstan)
library(rstantools)
library(bayesplot)
setwd("~/chisq_k1_500_prior2")
...
# compile and test
y<-synProcess500$y30
system.time(fit <- bayes_whittle(y,runs=1500,control=list(adapt_delta=0.5)))
fit
...
for (i in 1:1000) {
  ...
  saveRDS(res,"res_chisq_500_k1_bw.RDS")
