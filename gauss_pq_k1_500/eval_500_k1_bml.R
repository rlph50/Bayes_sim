...
synProcess <- readRDS('syn_k1gauss500.rds')
...
# compile and test
y<-synProcess$y22
system.time(fit <- bayes_ml(y,control=list(adapt_delta=0.9)))
fit
...
for (i in 1:1000) {
  ...
  saveRDS(res,"res_gauss_500_k1_ARMA_bml.RDS")
}


res <- readRDS("res_gauss_500_k1_ARMA_bml.RDS")
res$bml_adapt_delta <- 0.8

for (i in 1:1000) if (res$bml_divergent[i]>0) {
  cat(sprintf("processing y%d.\n",i))
  y<-synProcess[,paste0("y",i)]
