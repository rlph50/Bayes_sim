...
sim_optim<-function(par, objective, lower, upper, ...) {
  ...
  if (res$convergence!=0)
    res<-cobyla(par, objective, lower=lower, upper=upper, control=list(maxeval=5000), ...)
...
whittle.obj<-function(theta,ss) {
  ...
  spec_den_inv <- 2.0*pi * (4.0*((cos_2_pi_f-u1)^2))^fd1 * mod_phi / mod_theta  # Inverse of spectral density
...
whittle.est<-function(y) {
  ...
  fit<-sim_optim(c(0.25,spikes[['f1']],0.0,0.0),
                 whittle.obj,
                 lower=c(0.01,0.01,-0.999,-0.999),
                 upper=c(0.49,0.40, 0.999,0.999),
                 ss=ss)
  return(fit$par)
}

synProcess500 <- readRDS('syn_k1chisq500.rds')

res <- data.frame(whittle_d1=numeric(1000),whittle_f1=numeric(1000),
                  whittle_phi=numeric(1000), whittle_theta=numeric(1000), whittle_t=numeric(1000))
...
for (i in 1:1000) {
  ...
  res$whittle_theta[i]<-whittle[4]
  res$whittle_t[i]<-tt[['elapsed']]
  if ((i%%25)==0) cat(paste("running ",i,".\n"))
}

saveRDS(res,"res_chisq_500_k1_ARMA_whittle.RDS")
summary(res)
hist(res$whittle_phi,breaks=50)
hist(res$whittle_theta,breaks=50)
