...
# compile and test
y<-synProcess500$y1
system.time(fit <- bayes_whittle(y,control=list(adapt_delta=0.4)))
fit
...
for (i in 1:1000) {
  ...
  saveRDS(res,"res_chisq_500_k1_ARMA_bw.RDS")
  cat(paste("Finished ",i,".\n"))
}


res <- readRDS("res_chisq_500_k1_ARMA_bw.RDS")
for (i in 1:1000) if (res$bw_divergent[i]>0|res$bw_lp_rhat[i]>1.1) {
  y<-synProcess500[,paste0("y",i)]
